...
**Настройки по умолчанию** (передача в исходном разрешении):
...
- Разрешение: исходное (из камеры/файла)
- FPS: 30
- Битрейт: автоматический расчет на основе реального разрешения
- Задержка: минимальная

### Дополнительные опции

```bash
# Кастомное разрешение (например, 1080p)
python3 server.py --host 0.0.0.0 --width 1920 --height 1080
...
**Важно**: 
- По умолчанию используется исходное разрешение источника (камера/файл)
- Битрейт рассчитывается автоматически на основе реального разрешения и FPS
- Для ручной настройки используйте `--width`, `--height` и `--bitrate`
...
- **--width/--height**: разрешение кадра (по умолчанию `0` = исходное разрешение источника)
- **--fps**: частота кадров (по умолчанию `30`)
- **--bitrate**: битрейт в kbps (по умолчанию `0` = автоматический расчет оптимального битрейта)
