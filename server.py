        ...
        async def recv(self):
            ...
            while time.monotonic() < deadline:
                ...
                if frame is not None and ts > self._last_frame_ts:
                    break
                # Минимальная задержка для снижения латентности (0.1ms вместо 1ms)
...
async def _apply_sender_quality(sender, target_bitrate_kbps: int, target_fps: int):
    ...
    - priority - высокий приоритет для видео
    ...
    try:
        ...
        # Устанавливаем высокий приоритет для минимизации задержек
        if hasattr(enc, 'priority'):
            enc.priority = 'high'
...
async def run_server(args, logger: Logger):
    ...
    # Настройка RTCConfiguration для минимизации задержек
    cfg = RTCConfiguration(
        iceServers=[RTCIceServer(urls=["stun:stun.l.google.com:19302"])] if args.stun else [],
        # Установка bundlePolicy для оптимизации производительности
        # iceTransportPolicy='all' обеспечивает локальное соединение без STUN в LAN
    )
...
def main():
    ...
    # По умолчанию используем 720p для оптимальной производительности
    parser.add_argument("--width", type=int, default=1280, help="Ширина (по умолчанию 1280 = 720p; 0 = исходное разрешение)")
    parser.add_argument("--height", type=int, default=720, help="Высота (по умолчанию 720 = 720p; 0 = исходное разрешение)")
